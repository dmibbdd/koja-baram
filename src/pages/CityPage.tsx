  ...
  if (!city || city.countrySlug !== countrySlug) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-32 text-center">
        <div className="font-mono text-[#4A6741] text-xs tracking-widest uppercase mb-4">
          404 — NOT FOUND
        </div>

        <h1 className="font-serif text-[#1A1A18] text-3xl font-bold mb-4">
          Destination Not Found
        </h1>

        <p className="text-[#5A5A58] mb-8">
          The city you're looking for doesn't exist in our database. We never
          show placeholder data.
        </p>

        <Link
          to="/"
          className="bg-[#23412F] text-[#E6DDCB] font-mono text-xs px-6 py-3 hover:bg-[#2D5238] transition-colors"
          ...
          <div className="mt-4 flex items-end justify-between gap-4">
            <div>
              <div className="font-mono text-[#7A9E6B] text-xs tracking-wider mb-2">
                {city.coordinates.lat.toFixed(4)}° N &nbsp;
                {Math.abs(city.coordinates.lng).toFixed(4)}°{' '}
                {city.coordinates.lng > 0 ? 'E' : 'W'}
              </div>

              <h1
                className="font-serif text-[#F5F1E9] font-bold leading-tight"
                style={{
              ...
              <p className="text-[#C8B99A] text-lg mt-1">
                {tagline}
              </p>
            </div>
            ...
            <div className="hidden sm:flex items-center gap-2 shrink-0">
              <button
                ...
                className={`flex items-center gap-2 border px-4 py-2 text-sm font-mono transition-colors ${
                  saved
                    ? 'bg-[#23412F] border-[#4A6741] text-[#7A9E6B]'
                    : 'border-[#4A6741] text-[#C8B99A] hover:bg-[#1A2F1A]'
                }`}
                style={{ borderRadius: 4 }}
              >
                <svg
              ...
              <Link
                to="/ai-planner"
                className="bg-[#23412F] border border-[#4A6741] text-[#E6DDCB] px-4 py-2 text-sm font-mono hover:bg-[#2D5238] transition-colors"
                style={{ borderRadius: 4 }}
              >
                {t.planWithAI}
              </Link>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-[#2A3F5C]">
            {[
              ...
              <div key={label} className="px-5 py-4">
                <div className="font-mono text-[#4A6741] text-[10px] uppercase tracking-wider mb-0.5">
                  {label}
                </div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {city.galleryImages.map((img, i) => (
                    <button
                      ...
                      className={`relative aspect-[4/3] overflow-hidden bg-[#C8B99A] ${
                        galleryIndex === i
                          ? 'ring-2 ring-[#23412F]'
                          : ''
                      }`}
                ...
                <div className="grid grid-cols-3 gap-4">
                  {Object.entries(city.weather).map(([season, info]) => (
                    <div key={season} className="text-center">
                      <div className="font-mono text-[10px] text-[#7A9E6B] uppercase mb-1">
                        {season}
                      </div>

                      <div className="text-sm text-[#3A3A38]">
                        {info}
                      </div>
                    </div>
                  ))}
            ...
            <div className="space-y-4">
              <div className="field-card p-5">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div
                      key={label}
                      className="border-b border-[#E0D9C8] pb-3 last:border-0 last:pb-0"
                    >
                      <div className="font-mono text-[10px] text-[#7A9E6B] uppercase tracking-wider mb-0.5">
                        {label}
                      </div>
              ...
              <div className="field-card p-5">
                ...
                <ul className="space-y-2">
                  {(lang === 'fa'
                    ...
                    <li
                      key={i}
                      className="flex gap-2 text-sm text-[#3A3A38]"
                    >
                      <span className="font-mono text-[#C8B99A] shrink-0">
              ...
              <div className="field-card overflow-hidden">
                ...
                <div className="p-3 bg-[#F5F1E9]">
                  <p className="font-mono text-xs text-[#7A8575] text-center">
                    {name} — {country}
                  </p>
                </div>
              </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {city.neighborhoods.map((n) => (
                <div
                  key={n.id}
                  className="field-card overflow-hidden group"
                >
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between mb-2">
                      <h3 className="font-serif text-[#1A1A18] font-bold text-base">
                        {lang === 'fa' ? n.nameFa : n.name}
                      </h3>

                      <span
                        className="font-mono text-[9px] bg-[#E0D9C8] text-[#5A5A58] px-2 py-0.5 shrink-0"
          ...
          <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {city.restaurants.map((r) => (
                <div
                  key={r.id}
                  className="field-card group overflow-hidden hover:border-[#4A6741] transition-all"
                >
                  ...
                  <div className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-1">
                      <h3 className="font-serif text-[#1A1A18] font-bold">
                        {lang === 'fa' ? r.nameFa : r.name}
                      </h3>

                      <span className="font-mono text-xs text-[#C87941] shrink-0">
                        {r.priceRange}
                      </span>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <span
                        className="font-mono text-[10px] bg-[#E0D9C8] text-[#5A5A58] px-1.5 py-0.5"
                        style={{ borderRadius: 2 }}
                      >
                        {r.cuisine}
                      </span>

                      <span className="font-mono text-[10px] text-[#4A6741]">
                        ★ {r.rating}
                      </span>
                    </div>
        ...
        {activeTab === 'travelInfo' && (
          <div className="grid md:grid-cols-2 gap-6 max-w-3xl">
            ...
            <div className="field-card p-6">
              ...
              <p className="text-[#3A3A38] text-sm leading-relaxed">
                {lang === 'fa'
                  ? city.transportationFa
                  : city.transportation}
              </p>
            ...
            <div className="field-card p-6 md:col-span-2">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                {(lang === 'fa'
                  ...
                  <div
                    key={i}
                    className="flex gap-3 p-3 bg-[#F5F1E9] border border-[#E0D9C8]"
                    style={{ borderRadius: 4 }}
                  >
                    <span className="font-mono text-[#C8B99A] text-sm shrink-0">
                      {String(i + 1).padStart(2, '0')}
                    </span>
